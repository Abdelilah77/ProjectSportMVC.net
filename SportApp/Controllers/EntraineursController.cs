...
namespace SportApp.Controllers
{
    public class EntraineursController : Controller
    {
        ...
        {
            var test = Session["name"].ToString();
            var Dt = (from a in db.Entraineurs where a.Name == test select a.Sport).Distinct();
            ViewBag.sport = new SelectList(Dt.ToList());
            var ClientEntre = db.Entraineurs.SingleOrDefault(c => c.Name == test).Sport;
   
            
                
            
            return PartialView("_SeeClient2", db.Clients.Where(c => c.Sport== ClientEntre).ToList());
        ...
        {
            ViewBag.addoneajax = "add";
            var test = Session["name"].ToString();
            var Dt = (from a in db.Entraineurs where a.Name == test select a.Sport).Distinct();
            ViewBag.sport = new SelectList(Dt.ToList());
            var Tester = sportSearch_textresult(resulttext, sport, S1, S2, S3);
            return PartialView("_SeeClient2", Tester);
        }
        ...
        public PartialViewResult Details(int? id)
        {
            ...
            return PartialView("_Details2", client);
        }
        ...
        public PartialViewResult AddClient()
        {
            ...
            return PartialView("_AddClient2");
        }
        ...
        public PartialViewResult UpdateClient(int? id)
        {
            ...
            return PartialView("_UpdateClient2", client);
        }
        ...
        public PartialViewResult DeleteClient(int? id)
        {
            ...
            return PartialView("_DeleteClient2", client);
        }
